...
import { APIPromise } from 'octogen-api/core/api-promise';

import util from 'node:util';
import OctogenAPI from 'octogen-api';
import { APIUserAbortError } from 'octogen-api';
...
describe('instantiate client', () => {
  ...
  describe('logging', () => {
    ...
    beforeEach(() => {
      process.env = { ...env };
      process.env['OCTOGEN_API_LOG'] = undefined;
    });
    ...
    const forceAPIResponseForClient = async (client: OctogenAPI) => {
    ...
    test('default logLevel is warn', async () => {
      const client = new OctogenAPI({ octogenAPIKey: 'My Octogen API Key' });
    ...
    test('debug logs happen with debug env var', async () => {
      ...
      process.env['OCTOGEN_API_LOG'] = 'debug';
      const client = new OctogenAPI({ logger: logger, octogenAPIKey: 'My Octogen API Key' });
    ...
    test('warn when env var level is invalid', async () => {
      ...
      process.env['OCTOGEN_API_LOG'] = 'not a log level';
      const client = new OctogenAPI({ logger: logger, octogenAPIKey: 'My Octogen API Key' });
      expect(client.logLevel).toBe('warn');
      expect(warnMock).toHaveBeenCalledWith(
        'process.env[\'OCTOGEN_API_LOG\'] was set to "not a log level", expected one of ["off","error","warn","info","debug"]',
      );
    });
    ...
    test('client log level overrides env var', async () => {
      ...
      process.env['OCTOGEN_API_LOG'] = 'debug';
      const client = new OctogenAPI({ logger: logger, logLevel: 'off', octogenAPIKey: 'My Octogen API Key' });
  ...
  describe('baseUrl', () => {
    ...
    afterEach(() => {
      process.env['OCTOGEN_API_BASE_URL'] = undefined;
    });

    test('explicit option', () => {
      const client = new OctogenAPI({ baseURL: 'https://example.com', octogenAPIKey: 'My Octogen API Key' });
      expect(client.baseURL).toEqual('https://example.com');
    });

    test('env variable', () => {
      process.env['OCTOGEN_API_BASE_URL'] = 'https://example.com/from_env';
      const client = new OctogenAPI({ octogenAPIKey: 'My Octogen API Key' });
      expect(client.baseURL).toEqual('https://example.com/from_env');
    });

    test('empty env variable', () => {
      process.env['OCTOGEN_API_BASE_URL'] = ''; // empty
      const client = new OctogenAPI({ octogenAPIKey: 'My Octogen API Key' });
      expect(client.baseURL).toEqual('https://api.octogen.ai');
    });

    test('blank env variable', () => {
      process.env['OCTOGEN_API_BASE_URL'] = '  '; // blank
      const client = new OctogenAPI({ octogenAPIKey: 'My Octogen API Key' });
      expect(client.baseURL).toEqual('https://api.octogen.ai');
    });
    ...
    test('in request options overridden by env variable', () => {
      process.env['OCTOGEN_API_BASE_URL'] = 'http://localhost:5000/env';
      const client = new OctogenAPI({ octogenAPIKey: 'My Octogen API Key' });
      expect(client.buildURL('/foo', null, 'http://localhost:5000/option')).toEqual(
  ...
  test('maxRetries option is correctly set', () => {
    const client = new OctogenAPI({ maxRetries: 4, octogenAPIKey: 'My Octogen API Key' });
    expect(client.maxRetries).toEqual(4);

    // default
    const client2 = new OctogenAPI({ octogenAPIKey: 'My Octogen API Key' });
  ...
  test('with environment variable arguments', () => {
    ...
    process.env['OCTOGEN_API_KEY'] = 'My Octogen API Key';
    const client = new OctogenAPI();
    expect(client.octogenAPIKey).toBe('My Octogen API Key');
  });

  test('with overridden environment variable arguments', () => {
    // set options via env var
    process.env['OCTOGEN_API_KEY'] = 'another My Octogen API Key';
    const client = new OctogenAPI({ octogenAPIKey: 'My Octogen API Key' });
    expect(client.octogenAPIKey).toBe('My Octogen API Key');
  });
});
...
describe('retries', () => {
  test('retry on timeout', async () => {
    ...
    const client = new OctogenAPI({ octogenAPIKey: 'My Octogen API Key', timeout: 10, fetch: testFetch });
  ...
  test('retry count header', async () => {
    ...
    const client = new OctogenAPI({ octogenAPIKey: 'My Octogen API Key', fetch: testFetch, maxRetries: 4 });
  ...
  test('retry on 429 with retry-after', async () => {
    ...
    const client = new OctogenAPI({ octogenAPIKey: 'My Octogen API Key', fetch: testFetch });
